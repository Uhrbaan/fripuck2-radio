...
static void spp_service_setup(void) {
    ...
    for (int i = 0; i < NB_RFCOMM_CHANNELS; i++) {
        ...
        rfcomm_register_service_with_initial_credits(packet_handler, rf_channel[i].server_id, 0xffff,
                                                     INITIAL_INCOMMING_CREDITS);
        ...
        spp_create_sdp_record(rf_channel[i].spp_service_buffer, rf_channel[i].service_record, rf_channel[i].server_id,
                              channels_names[i]);
    ...
    if (xSemaphoreTake(channel->xWriteBluetooth, (TickType_t)DELAY_1000_TICKS) == pdTRUE) {
        if (channel->remote_id) {
            if (channel->remaining_size_blue_tx >= buffer_len) {
                ...
                log_info_rfcomm("remaining size = %d, nb_to_send = %d\n", channel->remaining_size_blue_tx,
                                channel->nb_to_send_blue_tx);
